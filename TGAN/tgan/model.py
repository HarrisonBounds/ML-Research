    ...
    def generator(self, z):
        ...
        * :math:`f_0` is a special vector :math:`\texttt{<GO>}` and we learn it during the
        ...
        with tf.compat.v1.variable_scope('LSTM'):
            cell = tf.keras.layers.LSTMCell(self.num_gen_rnn)

            state = cell.get_initial_state(self.batch_size)
            attention = tf.zeros(
                shape=(self.batch_size, self.num_gen_rnn), dtype='float32')
            input = tf.Variable(tf.random.normal(shape=(1, self.num_gen_feature)), name='go')  # <GO>
            ...
            for col_id, col_info in enumerate(self.metadata['details']):
                if col_info['type'] == 'value':
                    ...
                    with tf.compat.v1.variable_scope("%02d" % ptr):
                        ...
                        attw = tf.Variable("attw", shape=(len(states), 1, 1))
                    ...
                    with tf.compat.v1.variable_scope("%02d" % ptr):
                        ...
                        input = tf.concat([input, z], axis=1)
                        attw_initial_value = tf.ones((len(states), 1, 1))
                        attw = tf.Variable(attw_initial_value, name="attw")
                        attw = tf.nn.softmax(attw, axis=0)
                        attention = tf.reduce_sum(tf.stack(states, axis=0) * attw, axis=0)
